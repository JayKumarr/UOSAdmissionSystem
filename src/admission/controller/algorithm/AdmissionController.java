                                ...
                                if (dList.size() == 1) {
                                    ...
                                    if (pt.getIsBachelor()) {                                                                               /* Changed by JK from Percentage  */
                                        ...
                                        if (prerequisite.get(d.getPosId()) == null) {
                                            System.out.println("Pre-requisite not found : pos" + d.getPosId());
                                        }
            ...
            if (isMorning) {
                if (cn.getProvinceCode() == ProvinceEnum.SINDH.ordinal() && (cn.getDistrictId() != 22 && campusId != 7))/*22 of karachi & 7 for thatta campus*/ {
                    ...
                    if (cn.isContainCategory(CategoryEnum.GM_DUR_QUOTA) && cn.isCatNotAllocatedWithChoiceNo(isMorning, choiceNo, CategoryEnum.GM_DUR_QUOTA.ordinal())) {
                        /**
                         * GM, Upper Sindh, Female, Upper Female, Commerce
                         */
                            ...
                            if (cn.isJurisdication()) { // Jurisdiction
                                ...
                                //<editor-fold defaultstate="collapsed" desc="Other district seats (for badin campus where distributed by district not by urbal rural)">
                                    ...
                                    if (GenderEnum.values()[cn.getGender()].equals(GenderEnum.FEMALE) && INNER_F_MERIT.isSeatsAvailable()) {
                                        ...
                                        releaseSeat(catCode, cn);
                                        b = true;
                                    } else if (cn.getProgramGrp().contains("COM")) { // checking from COMMERCE GROUP AND SEATS
...
//                                    System.out.println(UPPER_MERIT);
                ...
                //</editor-fold>
                    ...
                    //</editor-fold>
//<editor-fold desc="SHUHDA Quota" defaultstate="collapsed">
                    if (cn.isContainCategory(CategoryEnum.SHUHDA_RES_QUOTA)) {
                        if (cn.isCatNotAllocatedWithChoiceNo(isMorning, choiceNo, CategoryEnum.SHUHDA_RES_QUOTA.ordinal())) {
                            CategoryAlg SHUHDA_MERIT = d.getCategoryMap().get(CategoryLogicalCodeEnum.SHUHDA_QUOTA);
                            if (SHUHDA_MERIT != null && SHUHDA_MERIT.isSeatsAvailable()) {
                                SHUHDA_MERIT.addCandidate(cn);
                                int catCode = cn.addCurrentSelection(isMorning, d.getPosId(), CategoryEnum.SHUHDA_RES_QUOTA.ordinal(), choiceNo, CategoryLogicalCodeEnum.SHUHDA_QUOTA.ordinal(), cnposId, SHUHDA_MERIT, d, cposId,this);
                                releaseSeat(catCode, cn);
                                status = choiceNo == 1 ? STATUS_FINAL_SELECTED : STATUS_PARTIAL_SHIFT_SELECTED;
                                cn.getCategories().remove(CategoryEnum.SHUHDA_RES_QUOTA);
                                break STATUS_LABEL;
                            }
                        }
                    }
                ...
                //<editor-fold defaultstate="collapsed" desc="If Army Person Quota">
                ...
                for (Integer campusId : campusIds) {
                    ...
                    for (Integer[] choice : choicesList) {
                        ...
                        if (dList.size() == 1) {
                            ...
                            /**
                             * Checking the prerequisite (candidate is
                             * applicable for selection in this discipline or
                             * not)
                             */
                            if (pt.getIsBachelor()) {                                                                               /* Changed by JK from Percentage  */
